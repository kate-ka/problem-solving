...
The function chooseBestSum (or choose_best_sum or ... depending on the language) will take as parameters t
 ...
 distances are positive or null integers and this list has at least one element). The function returns the "best"
...
def choose_best_sum(t, k, ls):
    sorted_combinations = sorted([sum(item) for item in combinations(ls, k)], reverse=True)
    selected_choices = list(dropwhile(lambda x: x > t, sorted_combinations))
    return selected_choices[0] if selected_choices else None
...
myTest.assertEqual(choose_best_sum(230, 4, xs), 230)
myTest.assertEqual(choose_best_sum(430, 5, xs), 430)
myTest.assertEqual(choose_best_sum(430, 8, xs), None)
