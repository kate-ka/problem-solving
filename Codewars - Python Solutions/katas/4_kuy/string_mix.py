...
We will only take into account the lowercase letters (a to z). First let us count
...
def mix(s1, s2):
    ...
    for letter in set(freq1.keys()) | set(freq2.keys()):
        freq_letter_s1 = freq1.get(letter, 0)
        freq_letter_s2 = freq2.get(letter, 0)

        if freq_letter_s1 > 1 or freq_letter_s2 > 1:
            if freq_letter_s1 > freq_letter_s2:
                maxis.append(('/1:', freq1[letter], letter))
            elif freq_letter_s1 < freq_letter_s2:
                maxis.append(('/2:', freq2[letter], letter))
            else:
                # Equal
                maxis.append(('/=:', freq2[letter], letter))

    maxis_2 = sorted(maxis, key=lambda x: (-x[1], x[0], x[2]))
    print(maxis_2)

    final_str = ''.join(item[0] + item[2] * item[1] for item in maxis_2)
    return final_str[1:]
